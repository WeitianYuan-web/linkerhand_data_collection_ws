...
class PiperRecorderAdapter:
    ...
    def _detect_available_can_interfaces(self) -> list:
            ...
            if result.returncode == 0:
                ...
                for line in result.stdout.strip().split('\n'):
                    if line.strip():
                        ...
                        iface = line.split()[0]
                        if iface.startswith('can') and iface[3:].isdigit():
                            interfaces.append(iface)
                return sorted(interfaces)
        except Exception as e:
            print(f"Warning: Could not detect CAN interfaces: {e}")
        return []
    
    def _load_can_config(self, side: str) -> str:
        """
        Load CAN configuration from hand_can_config.yaml.
        If configured interface doesn't exist, fallback to first available CAN interface.
            ...
            for parent in current_file.parents:
                ...
                if config_file.exists():
                    ...
                    if '/**' in config and 'ros__parameters' in config['/**']:
                        ...
                        configured_can = params.get(arm_key, '')
                        if configured_can:
                            break
        ...
        # Check if configured CAN interface exists
        if configured_can:
            available_interfaces = self._detect_available_can_interfaces()
            if configured_can in available_interfaces:
                return configured_can
            else:
                print(f"⚠️  Warning: Configured CAN interface '{configured_can}' not found. Available interfaces: {available_interfaces}")
        
        # Fallback: use first available CAN interface
        available_interfaces = self._detect_available_can_interfaces()
        if available_interfaces:
            fallback_can = available_interfaces[0]
            print(f"⚠️  Using fallback CAN interface: {fallback_can}")
            return fallback_can
        
        # Last resort: return default naming (may fail, but at least we tried)
        print(f"⚠️  Warning: No CAN interfaces detected, using default naming: arm_{side}")
        return f"arm_{side}"
