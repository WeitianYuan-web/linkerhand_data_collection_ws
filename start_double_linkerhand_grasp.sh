...
# 设置自研臂和遥控臂
# !!需要仔细阅读 README.md 文件，并注意进行标定。
cd ..
bash linker_master_arm/startup.sh
...
#创建双手采集任务
ros2 service call /linkerhand_data_collection_srv linkerhand_data_collection_srv/srv/Internal "req: '{\"method\": \"create_task\", \"id\": 121212, \"params\": {\"task_name\":\"double_linkerhand_grasp\",\"task_id\":2,\"task_configs_name\":\"double_linkerhand_grasp\"}}'"

# 开始录制（推荐）
bash scripts/manual_recording_control.sh start double_linkerhand_grasp
...
# 解压数据，进行查看
python3 scripts/extract_episode_data.py collection_data/double_linkerhand_grasp/session_2025-10-14_13-37-15/episode_000000